...
import JavaScriptQuestions from '../questions/jsQuestions';
...
const JavascriptQuiz = () => {
    ...
    const activeQuestion = JavaScriptQuestions[currentQuestion]
    ...
    const handleOptionSelection = (selectedId: number) => {
        ...
        setIsCorrect(selectedId === JavaScriptQuestions[currentQuestion].correct_option);
    };

    const nextQuestion = () => {
        if (JavaScriptQuestions.length - 1 === currentQuestion) {
            setIsCompleted(true);
            return;
        }
        if (isCorrect) {
        ...
        setCurrentQuestion((prevQuestion) =>
            prevQuestion < JavaScriptQuestions.length - 1 ? prevQuestion + 1 : prevQuestion
        );
    };
    ...
    return (
        <main className='mx-auto container max-w-xl px-4 pt-5'>
            <div className='flex justify-between items-center'>
                ...
                <h1 className='text-3xl text-cyan-950 font-semibold uppercase'>JS Quiz</h1>
            ...
            <section className='w-full'>
                <div>
                    <div className='text-center mt-10'>
                        ...
                        <motion.h4
                            key={JavaScriptQuestions[currentQuestion].question}
                            initial={{ opacity: 0, x: 100 }}
                            animate={{ opacity: 1, x: 0, transition: { ease: 'easeOut', delay: 0.4 } }}
                            transition={{ duration: 0.4, ease: "easeOut" }}
                            className='text-2xl text-cyan-900 mt-10 max-h-20 font-semibold'>{JavaScriptQuestions[currentQuestion].question}</motion.h4>
                    </div>
                    ...
                    <div className=" flex justify-between items-center my-14">
                        <h5 className='text-xl text-cyan-950 font-semibold '>Questions  {currentQuestion + 1}/{JavaScriptQuestions.length}</h5>
                        <Button type='button' disabled={!isSelected} variant="secondary" size="lg" onClick={nextQuestion}  >
                            {JavaScriptQuestions.length - 1 === currentQuestion ? 'Finish' : 'Next Question'}
                        </Button>
                    </div>
                </div>
            </section>
